...
public class EaseNotifier {
    ...
    public synchronized void onNewMsg(EMMessage message) {
        ...
        if (!IMApplication.getInstance().isMsgNotifyAllowed(message)) {
            return;
        }
    ...
    public synchronized void onNewMesg(List<EMMessage> messages) {
        ...
        if (!IMApplication.getInstance().isMsgNotifyAllowed(null)) {
            return;
        }
        // check if app running background
        if (!EasyUtils.isAppRunningForeground(appContext)) {
    ...
    public void vibrateAndPlayTone(EMMessage message) {
        ...
        try {
            ...
            if (IMApplication.getInstance().isMsgVibrateAllowed(message)) {
                long[] pattern = new long[]{0, 180, 80, 120};
                vibrator.vibrate(pattern, -1);
            }

            if (IMApplication.getInstance().isMsgSoundAllowed(message)) {
                if (ringtone == null) {
